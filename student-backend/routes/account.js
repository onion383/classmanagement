...
const { pickPhrase, deriveRestoreHash, formatPhrase, normalizePhrase } = require('../services/recoveryKey');
...
// v2 便携包：库钥匙用「密码派生钥匙」「助记词派生钥匙」各包一次放进包，
// 不再依赖服务端主密钥，换机后输密码或助记词任一个即可解包恢复。
router.get('/export', auth(), (req, res) => {
  try {
    const slug = req.user && req.user.dbSlug ? req.user.dbSlug : 'default';
    const { password, mnemonic } = req.query || {};
    if (!password && !mnemonic) {
      return res.status(400).json({ error: '请至少输入密码或助记词之一用于保护备份' });
    }
    ...
    if (password) out.wrappedByPassword = dbManager.wrapKeyBySecret(bundle.key, password);
    // 助记词先归一化再派生钥匙，与恢复侧一致（兼容带序号/空格的粘贴格式）
    if (mnemonic) out.wrappedByMnemonic = dbManager.wrapKeyBySecret(bundle.key, normalizePhrase(mnemonic));
    res.json(out);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
